 ...
 */
public class FaceManageServise {
    ...
    public static void AddFaceByBinary(int userID,String employeeNo ) throws JSONException, InterruptedException{
        ...
        int lHandler = UserAccessServe.hCNetSDK.NET_DVR_StartRemoteConfig(userID, HCNetSDK.NET_DVR_FACE_DATA_RECORD, ptrByteArray.getPointer(), strInBuffer.length(), null, null);
        if (lHandler < 0)
        {
            System.out.println("Addface NET_DVR_StartRemoteConfig 失败,错误码为"+UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
            return;
        }
        else{
            ...
            try{
                ...
                picfile = new FileInputStream(new File("..//AddFacePicture//FDLib.jpg"));
            }
            ...
            while(true){
                int dwState = UserAccessServe.hCNetSDK.NET_DVR_SendWithRecvRemoteConfig(lHandler, struAddFaceDataCfg.getPointer(), struAddFaceDataCfg.dwSize ,ptrOutuff.getPointer(), 1024,  pInt);
                //读取返回的json并解析
                ptrOutuff.read();
                ...
                if(dwState == -1){
                    System.out.println("NET_DVR_SendWithRecvRemoteConfig接口调用失败，错误码：" + UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
                    break;
                }
            ...
            if(!UserAccessServe.hCNetSDK.NET_DVR_StopRemoteConfig(lHandler)){
                System.out.println("NET_DVR_StopRemoteConfig接口调用失败，错误码：" + UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
            }
            else{
    ...
    public static void SearchFaceInfo(int userID,String  employeeNo) throws JSONException{
        ...
        int lHandler = UserAccessServe.hCNetSDK.NET_DVR_StartRemoteConfig(userID, HCNetSDK.NET_DVR_FACE_DATA_SEARCH, ptrByteArray.getPointer(), strInBuffer.length(), null, null);
        if (lHandler < 0)
        {
            System.out.println("SearchFaceInfo NET_DVR_StartRemoteConfig 失败,错误码为"+UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
            return;
        }
        else{
            ...
            while(true){
                int dwState =UserAccessServe.hCNetSDK.NET_DVR_SendWithRecvRemoteConfig(lHandler, ptrInbuff.getPointer(), strInbuff.length(), m_struJsonData.getPointer(), m_struJsonData.size(), pInt);
                m_struJsonData.read();
    ...
    public static void DeleteFaceInfo(int userID,String employeeNo)
    {
        ...
        String deleteFaceJson="{\n" +
                ...
                "    }]\n" +
                "}";
        String result=TransIsapiServise.put_isapi(userID,deleteFaceUrl,deleteFaceJson);
    ...
    public static void CaptureFaceInfo(int userID)
    {
        ...
        struCapCond.write();
        int lCaptureFaceHandle = UserAccessServe.hCNetSDK.NET_DVR_StartRemoteConfig(userID, HCNetSDK.NET_DVR_CAPTURE_FACE_INFO, struCapCond.getPointer(), struCapCond.size(), null, null);
        if (lCaptureFaceHandle == -1) {
            System.out.println("建立采集人脸长连接失败，错误码为" + UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
        ...
        struFaceInfo.read();
        while (true) {
            int dwState = UserAccessServe.hCNetSDK.NET_DVR_GetNextRemoteConfig(lCaptureFaceHandle, struFaceInfo.getPointer(), struFaceInfo.size());
            struFaceInfo.read();
            if (dwState == -1) {
                System.out.println("NET_DVR_GetNextRemoteConfig采集人脸失败，错误码：" + UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
                break;
            } else if (dwState == HCNetSDK.NET_SDK_CONFIG_STATUS_NEED_WAIT) {
        ...
        //采集成功之后断开连接、释放资源
        if (!UserAccessServe.hCNetSDK.NET_DVR_StopRemoteConfig(lCaptureFaceHandle)) {
            System.out.println("NET_DVR_StopRemoteConfig接口调用失败，错误码：" + UserAccessServe.hCNetSDK.NET_DVR_GetLastError());
        } else {
            System.out.println("NET_DVR_StopRemoteConfig接口成功");
        }
